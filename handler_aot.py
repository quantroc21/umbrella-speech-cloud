...
# --- AOT CACHE CONFIGURATION (PERSISTENT NETWORK VOLUME) ---
# Since we cannot bake GPU-specific kernels on GitHub Actions (CPU),
# we MUST use the Network Volume to persist them after the first run.
CACHE_ROOT = "/runpod-volume/.cache"
if os.path.exists("/runpod-volume"):
    print(f"AOT: Found Network Volume at /runpod-volume. Configuring Persistent Cache...")
    os.environ["TORCHINDUCTOR_CACHE_DIR"] = f"{CACHE_ROOT}/torch"
    os.environ["TRITON_CACHE_DIR"] = f"{CACHE_ROOT}/triton"
    os.environ["TORCHINDUCTOR_FX_GRAPH_CACHE"] = "1"
    
    # Ensure directories exist
    os.makedirs(f"{CACHE_ROOT}/torch", exist_ok=True)
    os.makedirs(f"{CACHE_ROOT}/triton", exist_ok=True)
else:
    print("AOT WARNING: No Network Volume found! Compilation will be slow and not persisted.")
...
print(f"--- INITIALIZING FISHSPEECH AOT HANDLER (v18.21-revert-18.7) ---")
