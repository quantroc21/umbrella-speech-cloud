...
try:
    ...
    print(f"--- [v12.8 DEBUG] Checking Volume Root: {VOLUME_PATH} ---", file=sys.stderr, flush=True)

    if os.path.exists(FISH_1_5_VOLUME):
        checkpoint_dir = FISH_1_5_VOLUME
        print(f"--- [v12.8 UPGRADE] Volume Status: READY (v1.5) ---", file=sys.stderr, flush=True)
        # v12: Pre-warm the volume files to RAM
        print(f"--- [v12.8 INIT] Pre-warming 4GB Model Weights (v1.5)... ---", file=sys.stderr, flush=True)
    else:
        # Fallback to local
    ...
    print(f"--- [v12.8 CONFIG] Llama: {LLAMA_CHECKPOINT_PATH} ---", file=sys.stderr, flush=True)
    print(f"--- [v12.8 CONFIG] Decoder: {DECODER_CHECKPOINT_PATH} ---", file=sys.stderr, flush=True)
            ...
            for i, paragraph in enumerate(paragraphs):
                ...
                for j, token in enumerate(chunks):
                    ...
                    if re.match(r'^(\.\.\.|[.!?;]+|[—,]|\- )$', token.strip()):
                        ...
                        logger.info(f"--- [v12.8 TRACE] Inference Start ---")
